...
def generar_pdf_espejo():
    ...
    # Encabezado (Lo que estaba en rojo en tu imagen)
    texto_intro = (f"En la ciudad de ROSARIO, departamento Rosario de la provincia de Santa Fe, a los {ahora.day} días del mes de {meses[ahora.month-1]} del año {ahora.year}, "
    ...
    pdf.multi_cell(190, 6, texto_intro)
    pdf.ln(4)
    ...
    pdf.multi_cell(190, 6, f"REQUISA: {requisa.upper()}", border=1)
    pdf.multi_cell(190, 6, f"SECUESTRO EN DEPÓSITO: {secuestro.upper()}", border=1)
    pdf.ln(4)
    
    pdf.multi_cell(190, 6, f"Se hace constar que se labra la presente en recibiendo de conformidad {of_recibe.upper()} en carácter de oficial de guardia. Con lo que no siendo para más se da por finalizado el presente acto del cual firman los testigos, demorado y el personal actuante para su debida constancia.")
    pdf.ln(10)
    pdf.cell(190, 10, f"HORA DE CESE: {hora_cese} hs. :-", ln=True)

    # Firmas corregidas (sin errores de sintaxis)
    pdf.ln(15)
    pdf.cell(63, 5, "________________", 0, 0, 'C')
    pdf.cell(63, 5, "________________", 0, 0, 'C')
    pdf.cell(63, 5, "________________", 0, 1, 'C')
    pdf.set_font('Arial', 'B', 8)
    pdf.cell(63, 5, "ACTUANTE", 0, 0, 'C')
...
if st.button("📄 GENERAR PDF"):
    if apellido and redacto:
        st.download_button("⬇️ DESCARGAR", data=generar_pdf_espejo(), file_name=f"10Bis_{apellido}.pdf")
