...
campos_base = {
    ...
    'lugar': "", 'hora_demora': datetime.now(import streamlit as st
from fpdf import FPDF
from datetime import datetime
...
with tabs[3]:
    ...
    # --- FUNCIÓN GENERADORA DE PDF (COMPATIBLE CON FPDF2) ---
    def crear_pdf():
        pdf = FPDF()
        ...
        cuerpo = (
            ...
            f"Adoptándose esta medida por el motivo de que ante la presencia policial: {st.session_state.motivo_unico.upper()}. "
            ...
            f"c) no será incomunicado; d) tiene derecho a realizar una llamada telefónica; e) no será alojado con detenidos comunes; "
            f"f) se labra la presente acta ante los testigos: {st.session_state.testigo_datos.upper()}. Se hace constar que de la requisa efectuada sobre el demorado "
            f"la misma arrojó resultado {st.session_state.requisa.upper()}. Es dable hacer mención que se le secuestra en carácter de depósito para su resguardo: "
            f"{st.session_state.secuestro.upper()}. Se deja constancia que el demorado {st.session_state.estado_fisico}. "
            f"Se hace constar que se labra la presente en recibiendo de conformidad {st.session_state.of_recibe.upper()} en carácter de oficial de guardia. "
        ...
        # Corrección crítica para fpdf2: devuelve bytes directamente
...
with tabs[4]:
    ...
    for i, reg in enumerate(reversed(st.session_state.historial)):
        if st.button(f"Cargar: {reg['apellido']} ({reg['hora_demora']} hs)", key=f"h_{i}"):
            ...
            st.rerun()).strftime('%H:%M'), 'acta_nro': "",
    'apellido': "", 'nombre': "", 'dni': "", 'nacionalidad': "ARGENTINA",
    'est_civil': "SOLTERO/A", 'edad': "", 'nacimiento': "", 'domicilio': "", 'padres': "",
    'actuante_ap': "CASTAÑEDA", 'actuante_nom': "JUAN", 'refuerzo_ap': "GOMEZ", 'refuerzo_nom': "RICARDO",
    'testigo_datos': "", 'requisa': "PALPADO PREVENTIVO SOBRE SUS PRENDAS NO HALLANDO ELEMENTOS DE PELIGROSIDAD", 
