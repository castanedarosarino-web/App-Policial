...
with tab3:
    ...
    st.session_state.testigo_datos = st.text_area("Testigos (Nombre, DNI, Dom)", value=st.session_state.testigo_datos)
    st.session_state.requisa = st.text_area("Requisa Personal", value=st.session_state.requisa)
    st.session_state.secuestro = st.text_area("Secuestro en Depósito", value=st.session_state.secuestro)
...
with tab4:
    ...
    st.session_state.motivo_final = st.text_area("Motivo Final:", value=seleccion if seleccion != "OTRO (Manual)" else st.session_state.motivo_final)
    
    st.session_state.of_recibe = st.text_input("Oficial de Guardia que recibe (Rango y Apellido)", value=st.session_state.of_recibe)

    # --- GENERADOR PDF ESPEJO TOTAL ---
    def generar_pdf():
        pdf = FPDF()
        ...
        # Estado Físico y CIERRE SOLICITADO
        pdf.set_font('Arial', '', 11)
        pdf.ln(5)
        ...
        # EL CIERRE QUE FALTA
        cierre_texto = (f"Se hace constar que se labra la presente en recibiendo de conformidad {st.session_state.of_recibe.upper()} en carácter de oficial de guardia. "
                        f"Con lo que no siendo para más se da por finalizado el presente acto del cual firman los testigos, demorado y el personal actuante para su debida constancia.")
        pdf.multi_cell(165, 8, cierre_texto, align='J')
        
        pdf.ln(10)
        pdf.cell(165, 10, f"HORA DE CESE: __________ hs.", ln=True)
        return pdf.output(dest='S').encode('latin-1')

    st.download_button("📄 DESCARGAR PDF (CON CIERRE COMPLETO)", data=generar_pdf(), file_name=f"10Bis_{st.session_state.apellido}.pdf")
...
*MOTIVO:* {st.session_state.motivo_final[:50]}...
...
with tab5:
    ...
    for i, reg in enumerate(reversed(st.session_state.historial)):
        ...
        if st.button(f"📌 {reg['apellido']} ({reg['fecha_registro']})", key=f"h_{idx}"): recuperar_registro(idx)
