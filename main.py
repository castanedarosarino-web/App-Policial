...
with tab4:
    ...
    def generar_pdf():
        # Clase personalizada para manejar negritas intercaladas sin romper el justificado
        class PDF(FPDF):
            def multi_cell_justified_bold(self, w, h, segments):
                # Esta lógica simula el justificado alternando fuentes
                # Para evitar errores de librería, usamos multi_cell estándar
                # Pero concatenamos el texto para que mantenga el estilo espejo
                full_text = ""
                for text, is_bold in segments:
                    full_text += text
                self.set_font('helvetica', '', 11)
                self.multi_cell(w, h, full_text, align='J')
        ...
        # TÍTULO (Espejo del Word) [cite: 1]
        pdf.set_font('helvetica', 'B', 12)
        pdf.cell(0, 10, "ACTA DE DEMORA ART 10 BIS LEY 7.395", new_x="LMARGIN", new_y="NEXT", align='C')
        pdf.ln(5)

        # CUERPO DEL ACTA (Justificado y con datos en mayúsculas/negritas donde corresponde) [cite: 2, 4]
        ...
        # SECCIÓN DE FIRMAS (Espejo del modelo Word) [cite: 15, 16]
        pdf.ln(20)
...
with tab5:
    st.subheader("🗂️ Historial de Sesión")
